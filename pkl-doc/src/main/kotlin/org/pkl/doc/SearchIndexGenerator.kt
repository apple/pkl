...
internal class SearchIndexGenerator(private val outputDir: Path) {
  ...
  private fun List<SearchIndexEntry>.writeTo(path: Path) {
    path.outputStream().use { stream ->
      stream.write(PREFIX.toByteArray(StandardCharsets.UTF_8))
  ...
  private fun ModuleData.toEntry(basePath: String): SearchIndexEntry {
    return SearchIndexEntry(
      name = ref.fullName,
      kind = Kind.MODULE,
      url = "$basePath/${ref.module.pathEncoded}/index.html",
      deprecated = deprecation?.let { true },
    )
  }
  ...
  internal fun getCurrentSearchIndex(): List<PackageIndexEntry> {
    ...
    val text = searchIndexFile.readText()
    if (!(text.startsWith(PREFIX) && text.endsWith(POSTFIX))) {
      System.err.println(
        "Incorrect existing search-index.js; either doesnt start with prefix '$PREFIX', or end with postfix '$POSTFIX'"
      )
      return emptyList()
    }
  ...
  fun buildSearchIndex(newlyGeneratedPackages: List<PackageData>): List<PackageIndexEntry> {
    val searchIndex = getCurrentSearchIndex().toMutableList()
    for (pkg in newlyGeneratedPackages) {
      val pkgPath = "${pkg.ref.pkg.pathEncoded}/current"
      val packageIndexEntry =
        PackageIndexEntry(
          packageEntry = pkg.toEntry(),
          moduleEntries = pkg.modules.map { it.toEntry(basePath = pkgPath) },
        )
      val currentPkgIdx = searchIndex.indexOfFirst { it.packageEntry.name == pkg.ref.pkg }
      if (currentPkgIdx >= 0) {
        searchIndex[currentPkgIdx] = packageIndexEntry
      } else {
        searchIndex.add(packageIndexEntry)
      }
    }
    return searchIndex.sortedBy { it.packageEntry.name }
  }

  /** Reads the current site index, and adds the set of newly generated packages to the index. */
  fun generateSiteIndex(searchIndex: List<PackageIndexEntry>) {
