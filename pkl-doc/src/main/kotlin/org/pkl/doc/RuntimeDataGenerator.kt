...
internal class RuntimeDataGenerator(
    ...
    private fun writeModuleFile(ref: ModuleRef) {
        outputDir
            .resolve("data/${ref.pkg}/${ref.version}/${ref.module}/index.js")
            .jsonWriter()
            .use { writer ->
                writer.isLenient = true
                writer.writeLinks(
                    HtmlConstants.KNOWN_VERSIONS,
                    moduleVersions
                        .getOrDefault(ref.id, setOf())
                        .sortedWith(descendingVersionComparator),
                    { it },
                    {
                        if (it == ref.version) null
                        else ref.copy(version = it).pageUrlRelativeTo(ref)
                    },
                    { if (it == ref.version) CssConstants.CURRENT_VERSION else null }
                )
                writer.writeLinks(
                    HtmlConstants.KNOWN_USAGES,
                    typeUsages
                        .getOrDefault(ref.moduleClassRef, setOf())
                        .typesWithHighestVersions()
    ...
    private fun Set<PackageRef>.packagesWithHighestVersions(): Collection<PackageRef> {
        ...
        for (ref in this) {
            ...
            if (
                prev == null || descendingVersionComparator.compare(prev.version, ref.version) > 0
            ) {
