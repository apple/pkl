...
internal sealed class DocScope {
    ...
    /**
     * Looks up the property or class with the given name in the program element associated with
     * this scope.
     */
    ...
    override fun resolveVariable(name: String): DocScope? =
        name.takeIf { it == "module" }?.let { this }
            ?: module.imports[name]?.let { uri ->
                val mod = resolveImport(uri)
                resolveModuleNameToDocUrl(mod.moduleName)?.let { url ->
                    ModuleScope(mod, url, null)
                }
            }
