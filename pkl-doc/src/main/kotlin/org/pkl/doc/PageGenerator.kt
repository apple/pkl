...
internal abstract class PageGenerator<out S>(
    ...
    protected fun HTML.renderHtmlHead() {
        head {
            ...
            script {
                src =
                    pageScope.relativeSiteUrl.resolve("scripts/scroll-into-view.min.js").toString()
                defer = true
            }
    ...
    private val PClass.simpleDisplayName: String
        get() =
            if (isModuleClass) moduleName.substring(moduleName.lastIndexOf('.') + 1) else simpleName
    ...
    private fun HtmlBlockTag.renderTypeArguments(
        typeArguments: List<PType>,
    ...
    protected fun HtmlBlockTag.renderModifiers(
        ...
        if (examples.isNotEmpty() && docModule.parent.docPackageInfo.sourceCodeUrlScheme != null) {
            result[MemberInfoKey("Examples")] = {
                ...
                for (example in examples) {
                    ...
                    a {
                        href =
                            docModule.parent.docPackageInfo.getModuleSourceCode(
                                example.moduleName
                            )!!
    ...
    protected inner class MemberDocs(
        ...
        private val overflow: String? =
            if (showSummaryOnly) {
                ...
                docComment
                    ...
                    ?.let {
                        markdownRenderer.render(markdownParser.parse(it)).trim().ifEmpty { null }
                    }
            }
        ...
        val isExpandable: Boolean =
            !showSummaryOnly &&
                (overflow != null || summaryMovesDown || hasMemberInfo && !isDeclaration)
                ...
                tag.dl {
                    ...
                    if (alsoKnownAsAnnotation != null) {
                        ...
                        dd {
                            ...
                            val names = alsoKnownAsAnnotation["names"] as List<String>
                            var first = true
                            for (name in names) {
                                if (first) first = false else +", "
                                code { +name }
