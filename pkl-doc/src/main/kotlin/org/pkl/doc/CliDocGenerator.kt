...
class CliDocGenerator(private val options: CliDocGeneratorOptions) : CliCommand(options.base) {
    ...
    private fun DependencyMetadata.getPackageDependencies():
        List<DocPackageInfo.PackageDependency> {
        return buildList {
            for ((_, dependency) in dependencies) {
                val metadata =
                    try {
                        packageResolver.getDependencyMetadata(
                            dependency.packageUri,
    ...
    override fun doRun() {
        ...
        // Evaluate module imports eagerly, which is cheap if docs are also generated for most
        // imported
        // modules.
        ...
        try {
            ...
            builder.build().use { evaluator ->
                ...
                docsiteInfo =
                    when {
                        docsiteInfoModuleUris.isEmpty() -> DocsiteInfo(null, null, mapOf())
                            ...
                            val module =
                                evaluator.evaluate(ModuleSource.uri(docsiteInfoModuleUris.single()))
                ...
                for (uri in regularModuleUris) {
                    val entry =
                        schemasByDocPackageInfoAndPath.keys.find {
                            uri.path.startsWith(it.second.toString())
                        }
                            ?: throw CliException(
