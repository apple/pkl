...
class CliDocGeneratorTest {
    companion object {
        ...
        private val expectedOutputFiles: List<Path> by lazy {
            expectedOutputDir.listFilesRecursively()
        }
    ...
    fun `cannot have multiple docsite descriptors`() {
        ...
        val generator =
            CliDocGenerator(
                CliDocGeneratorOptions(
                    CliBaseOptions(
                        sourceModules = listOf(descriptor1.toUri(), descriptor2.toUri())
                    ),
    ...
    fun `current() excludes prerelease versions`() {
                ...
                DocPackage(
                    docPackageInfo =
                        DocPackageInfo(
                            ...
                            sourceCodeUrlScheme =
                                "https://example.com/blob/$version%{path}#L%{line}-%{endLine}"
                        ),
                    modules = emptyList()
                ),
            )
        ...
        val comparator =
            Comparator<String> { v1, v2 -> Version.parse(v1).compareTo(Version.parse(v2)) }
                .reversed()
