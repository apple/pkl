...
class SearchTest {
        ...
        private val jsContext = lazy {
            ...
            Context.newBuilder("js").allowAllAccess(true).build().apply {
                eval(
                    Source.create(
                        "js",
                        pkldocDir.resolve("com.package1/1.2.3/search-index.js").readString()
                    )
                )
                eval(Source.create("js", pkldocDir.resolve("scripts/pkldoc.js").readString()))
                eval(
                    Source.create("js", pkldocDir.resolve("scripts/search-worker.js").readString())
                )
            }
        }
    ...
    fun `search with full name`() {
        checkSearchResults(
            ...
            setOf("com.package1.modulePropertyAnnotations")
        )
    }
    ...
    fun `exact match of last module name component is considered exact match`() {
        checkExactSearchResults(
            "modulePropertyComments",
            setOf("com.package1.modulePropertyComments")
        )
    }
