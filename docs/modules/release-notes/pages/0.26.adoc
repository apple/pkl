...
HTTP proxy settings are also added as new fields in the xref:bindings-specification:message-passing-api.adoc#create-evaluator-request[message passing API].
Authors of external Pkl clients are encouraged to support these new fields to provide proxy support in their libraries.

To read more about proxing, consult the xref:pkl-cli:index.adoc#http-proxy[documentation].
...
On the other hand, JVM library users typically interact with Pkl by bundling Pkl files a classpath resources, and load them using `modulepath:` URIs.
...
Multiple changes have been more to the standard library.
...
This module is expected to `amends "pkl:settings"`, but this behavior is not checked.

In 0.26, it will be an error if the settings module does not amend `"pkl:settings"`, nor set its `output.value` to an instance of the settings module.
