...
For new setups, the CLI no longer stores anything under `~/.pkl`. It uses XDG-style locations:
...
* `org.pkl.core.settings.PklSettings.loadFromPklHomeDir()` – renamed to `PklSettings.loadFromDefaultLocation()`, which now prefers `~/.config/pkl/settings.pkl` over the legacy `~/.pkl/settings.pkl`.
