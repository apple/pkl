...
By default, the CLI no longer stores anything under `~/.pkl`. It now uses XDG-style locations:
...
Existing setups keep working without migration: each legacy `~/.pkl` location is still used when it already exists. In particular, a pre-existing `~/.pkl/cache` is still used, so packages aren't re-downloaded.
