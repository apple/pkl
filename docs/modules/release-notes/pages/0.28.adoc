...
Improvements have been made to the Java/Kotlin code generators (https://github.com/apple/pkl/pull/792[#792], https://github.com/apple/pkl/pull/793[#793]).

Thanks to https://github.com/odenix[@odenix] for their contribution!

==== New `--params-annotation=none` argument

In pkl-codegen-java, the `--params-annoatation` flag now accepts `none` as a value, which causes the code generator to skip adding annotations to a constructor's parameters.
...
==== Consistent `toString()` representation
