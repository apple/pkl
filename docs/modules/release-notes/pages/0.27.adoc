...
=== `Mapping`/`Listing` typecheck improvements

Typechecking of `Listing` and `Mapping` types has been improved (https://github.com/apple/pkl/pull/628[#628], https://github.com/apple/pkl/pull/725[#725], https://github.com/apple/pkl/pull/740[#740], https://github.com/apple/pkl/pull/752[#752]).

Today, in order to typecheck `Mapping` and `Listing`, Pkl will execute each member to check it against the type parameter

For example, in order to typecheck a value as `Listing<Bird>` (given that `Bird` is a class), the evaluator runs the following steps:

1. Check that the value is an instance of `Listing`
2. For each listing member, check that it is an instance of `Bird`

This behavior is different from how the rest of Pkl works.
Generally, Pkl will only execute code that is in the critical path of evaluation.
...
Even though `canFly` has received an invalid value, this program evaluates to `birdName = "Pidgy"`. This is because `canFly = throw("uh oh")` is not executed.

The behavior of `Mapping` and `Listing` has been changed to match.
This means that their members are not executed if they are not in the critical path of evaluation.

This subtly changes the behavior of some existing code.
...
----
<1> In Pkl 0.26 and below, throws a type mismatch exception.
+
In Pkl 0.27, yields `1`.

In practice, this means that some Pkl programs will execute more quickly.
For example, this program effectively only imports one module called `foo`.

In Pkl 0.26 and below, this results in _every_ import being evaluated.
In Pkl 0.27, only one import is evaluated.
...
----
<1> Perform just one logical import by selecting it from a glob import result. This is a common pattern for "dynamic" imports.
...
A new feature has been added to allow Pkl to spawn external processes to read resources and modules (https://github.com/apple/pkl/pull/660[#660]).
...
The Pkl CLI will now emit some messages in color (https://github.com/apple/pkl/pull/552[#552], https://github.com/apple/pkl/pull/746[#746], https://github.com/apple/pkl/pull/771[#771]).
...
* Skip generation of `copy()`, `equals()`, `hashCode()`, `toString()`, `copy()` methods for abstract Kotlin classes.
* Don't make abstract classes implement `Serializable`
* Add support for Spring Boot 3.
...
|Moved to `org.pkl.core.TestResults`, turn into a record, and largely restructured.
...
If Spring Boot 2 is needed, the code generator from Pkl 0.26 can be used.
...
Due to changes to <<typecheck-improvements,Listing and Mapping typechecking>>, the behavior of Pkl programs has altered slightly.

Because Pkl now performs lazy typechecks on these objects, some programs that used to throw do not throw anymore.
...
* CLI `--property` flags containing `=` in property values are not parsed correctly (https://github.com/apple/pkl/pull/631[#631]).
* Thrown `PklBugException` when reading assets from a local project dependency (https://github.com/apple/pkl/pull/642[#642]).
...
* Kotlin generator fails with "copy overrides nothing" for subclasses of abstract classes (https://github.com/apple/pkl/issues/569[#569]).
...
* Type parameters new `new Mapping<TypeA, TypeB>`/`new Listing <Type>` are not checked (https://github.com/apple/pkl/issues/405[#405]).
