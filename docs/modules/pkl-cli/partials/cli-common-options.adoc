...
[%collapsible]
====
Default: `~/.cache/pkl` (or the legacy `~/.pkl/cache` if it already exists) +
...
If not set, `~/.config/pkl/settings.pkl` (or the legacy `~/.pkl/settings.pkl`), or defaults specified in the `pkl.settings` standard library module are used.
====
