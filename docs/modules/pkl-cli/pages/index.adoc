...
* Java executable (tested with Java 8/11/14 on macOS and Oracle Linux, may work on other platforms)
...
Native Windows is not a currently supported platform for Pkl however you may be able to still achieve some level of success.  Official support is planned for the next release (targetting June timeframe).  Pkl does work on Windows through https://learn.microsoft.com/en-us/windows/wsl/install[WSL] but our Java Executables have some degree of compatibility with Windows native.

The following instructions should work, but again no official support is provided:

* You will need a Java Runtime installed (ie https://www.oracle.com/java/technologies/downloads/#jdk21-windows[Java SE / JDK 21] )
* You need to download our JAR files for the CLI this can be found in our {uri-ci-artifacts}[Sonatype Repository] or from https://search.maven.org/remote_content?g=org.pkl-lang&a=pkl-cli-java&v=LATEST[Maven]
* You can then run the Java Executable and see the following result:
