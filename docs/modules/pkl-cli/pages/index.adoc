...
*Synopsis*: `pkl format check <paths>`

This command checks for format violations on the given files and print their names to stdout. +
It returns a non-zero status code in case violations are found.

[[command-format-apply]]
=== `pkl format apply`

*Synopsis*: `pkl format apply [<options>] <paths>`
