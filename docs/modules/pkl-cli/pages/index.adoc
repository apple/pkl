...
<module>::
The absolute or relative URIs of the command module to run. Relative URIs are resolved against the working directory.
...
These tools are executed by running `pkl run`.
...
* Commands can produce output to standard output and written files at the same time.
* Like CLI tools written in other languages, command file output may write to any absolute path (not just relative to the `--multiple-file-output-path` option).
* Commands provide a way to dynamically import modules when they are specified as command line options.

Commands are implemented as regular modules and declare their support command line flags and positional arguments using a class with annotated properties.
...
When commands are executed, the Pkl runtime parses and validates provided options and synthesizes a module that amends the command's.
This synthesized module overrides the `options` property to set the parsed value for each option.
This module is evaluated and its `output.bytes` property is written to the standard output and files specified by `output.files` are written to the filesystem.

=== Command Options

Each property of the class of a command's `options` property becomes a command line options.
...
Properties may be annotated with link:{uri-stdlib-Command-Flag}[`@Flag`] or link:{uri-stdlib-Command-Argument}[`@Argument`] to influence how they behave:
...
* An xref:language-reference:index.adoc#anonymous-functions[anonymous function] that accepts the raw string value passed on the command line and returns a value of the property's annotated type.
...
* The special value `"import*"`, which treats the option's value as a xref:language-reference:index.adoc#globbed-imports[globbed module URI] and sets the option's value to a link:{uri-stdlib-Mapping}[`Mapping`] with keys that are the resolved absolute module URIs and values that are the imported module contents. When specifying glob import options, it will often be necessary to quote the value to avoid it being interpreted by the shell.
...
// Regular module code...
----
<1> Optional; asserts that this is a subcommand of `my-tool` and simplifies accessing parent command properties and options

=== Testing Commands

Command module are normal Pkl modules, so they may be imported and used like any other module.
