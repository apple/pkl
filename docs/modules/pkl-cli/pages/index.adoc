...
Its default location is `~/.config/pkl/settings.pkl`, falling back to the legacy `~/.pkl/settings.pkl` if that exists.
...
- Add them to a directory at path `~/.config/pkl/cacerts/` (or the legacy `~/.pkl/cacerts/`).

Both these options will *replace* the default CA certificates bundled with Pkl. +
The CLI option takes precedence over the certificates in `~/.config/pkl/cacerts/`. +
