...
:uri-certificates: {uri-github-tree}/pkl-commons-cli/src/main/resources/org/pkl/certificates
...
This option is used for embedding Pkl in an external client, such as xref:go:ROOT:index.adoc[pkl-go].
...
Prior to publishing, this command will run the package's API tests if any are defined.
