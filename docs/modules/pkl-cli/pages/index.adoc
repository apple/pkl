...
*Synopsis*: `pkl format <options> <file-or-dir-path>`
...
By default, if a file has violations, write the formatted version of said
file to stdout.
...
[%collapsible]
====
Don't write to stdout or stderr. Mutually exclusive with `--names`.
====

.-w, --write
[%collapsible]
====
Format files in place, overwriting them. Implies `--names`.
====
