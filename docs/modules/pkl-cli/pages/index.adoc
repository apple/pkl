...
* Commands can produce output to standard output (via `output.text` or `output.bytes`) and written files (via `output.files`) at the same time.
...
Pkl provides no facility for making arbitrary HTTP requests (i.e. non-`GET` methods or setting headers), executing other programs, or any other side effect.
