...
This is done on the command line by passing `--external-resource` and `--external-module` flags, which may both be passed multiple times.

[source,text]
----
$ pkl eval <module> --external-resource <scheme>=<executable> --external-module <scheme>='<executable> <argument> <argument>'
----
