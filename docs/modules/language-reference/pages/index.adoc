...
`Bytes` represent binary data, and is typically rendered differently. For example, they are rendered as `<data>` tags when using `PListRenderer`.
...
For example, concatenating two `Bytes` values allocates O(M + N) space, whereas concatenating two `List` values allocates O(1) space.
