...
:uri-pkl-go-resource-reader-docs: https://github.com/pkl-lang/pkl-go/blob/main/pkl/reader.go
...
For a hands-on introduction, see xref:language-tutorial:index.adoc[Language Tutorial].
...
its species will be remembered.
"""
----
...
[source%tested,{pkl}]
----
strLength = "abc".length // <1>
reversedStr = "abc".reverse() // <2>
hasAx = "abc".contains("ax") // <3>
trimmed = "  abc  ".trim() // <4>
----
<1> result: `3`
<2> result: `"cba"`
...
Let's define an object with properties `name` and `isExtinct`:
...
----
<1> Defines a module property named `pigeon`.
    The open curly brace (`{`) indicates that the value of this property is an object.
<2> Defines an object property named `name` with string value `"Pigeon"`.
<3> Defines an object property named `isExtinct` with boolean value `false`.
...
pigeonIsExtinct = pigeon.isExtinct
----
...
----
<1> Defines an object property named `profile`.
    The open curly brace indicates that its value is another object.

As you probably guessed, the nested property `age` can be accessed with `pigeon.profile.age`.
...
----
<1> Defines a module property named `pigeon2`.
    Its value is an object that _amends_ `pigeon`.
    Note that the amended object must be enclosed in parentheses.
<2> Object property `pigeon2.profile` _amends_ `pigeon.profile`.
<3> Object property `pigeon2.profile.age` _overrides_ `pigeon.profile.age`.
...
alarm {
  lowThreshold = 100
  highThreshold = lowThreshold * 3 // <1>
}
highThreshold = alarm.highThreshold
----
<1> result: `300`

We have defined a hypothetical `alarm` object whose `highThreshold` property is defined in terms of `lowThreshold`.
Can you guess what happens when `alarm` is amended and its `lowThreshold` overridden?
...
highThreshold = alarm2.highThreshold // <1>
----
<1> result: `600`

As you can see, ``alarm2``'s `highThreshold` changed along with its `lowThreshold`.
...
The resulting dynamic object is equivalent to `pigeon`, except that it no longer has a `name` property.
...
* A link:{uri-stdlib-Dynamic}[Dynamic] object has no predefined structure:footnote:[By "structure" we mean a list of property names and (optionally) property types.].
...
====
* Use typed objects to build schema-backed data models that are validated.
  footnote:[By "Use typed objects" we mean to define classes and build data models out of instances of these classes.]
...
----
<1> Defines a class named `Bird` with properties `name`, `age` and `isExtinct`.
...
Congratulations, you have constructed your first typed object!
footnote:[Not counting that every module is a typed object.]
...
class Bird {
  name: String
  age: Int
  hidden nameAndAgeInIndex = "\(name), \(age)" // <1>
  nameSignWidth: UInt = nameAndAgeInIndex.length // <2>
}
...
pigeonInIndex = pigeon.nameAndAgeInIndex // <4>
...
class Bird {
  ...
  age: Int
  local separator = "," // <1>
  hidden nameAndAgeInIndex = "\(name)\(separator) \(age)" // <2>
}
...
pigeonInIndex = pigeon.nameAndAgeInIndex // <3>
...
<3> The _value_ of `separator` occurs in `nameAndAgeInIndex`.
...
fixed examples: Listing<String> = new {
  "Pigeon"
  "Hawk"
  "Penguin"
}
...
<2> Error: cannot amend fixed property `examples`

When extending a class and overriding an existing property definition, the fixedity of the overridden property must be preserved.
...
class Add {
  a: Int
  b: Int
  fixed result: Int = a + b
}
----

Another use-case for `fixed` is to define properties that are meant to be fixed to a class definition.
In the example below, the `scientificName` of a bird is tied to the class, and therefore is declared `fixed`.
...
----
abstract class Bird {
  fixed scientificName: String // <1>
}

class Osprey extends Bird {
  fixed scientificName: "Pandion haliaetus" // <2>
}
----
<1> No explicit default because the property is overridden by a child class.
<2> Overrides the type from `String` to the <<string-literal-types,string literal type>> `"Swallow"`. +
...
const function birdAge(i: Int): Int = (i / 4).toInt()
...
<2> Allowed: `birdAge` is const.

Within a class or annotation body, references to members of its enclosing module must be `const`.
NOTE: This rule is in place because it would otherwise change a class or annotation's definition when amending its enclosing module.
...
class Bird {
  ...
  age: Int = birdAge(24) // <2>
}

@Deprecated { message = "Replace with \(pigeonName)" } // <3>
oldPigeonName: String
----
<1> Error: cannot reference non-const property `pigeonName` from a class body.
<2> Allowed: `birdAge` is const.
<3> Error: cannot reference non-const property `pigeonName` from an annotation body.

Values from an <<import-clause,import clause>> are implicitly `const`.
Therefore, it is okay to reference these values as if they were `const`.
...
class Circle {
  radius: Int
  fixed area: Float = math.pi * radius * radius // <1>
}
----
<1> `math.pi` is allowed here, because it comes from an import clause.
...
This concludes our tour of objects, one of the most complex but also most powerful features of Pkl.
...
----
<1> Amends the `default` element and sets property `age`.
<2> Defines a new listing element that implicitly amends the default element.
<3> Defines a new property called `name`. Property `age` is inherited from the default element.
<4> Defines a new listing element that implicitly amends the default element.
<5> Overrides the default for property `age`.
...
----
<1> Defines a local property name `pigeon` with value `"Pigeon"`.
    Local properties can have a type annotation, as in `pigeon: String = "Pigeon""`.
<2> Defines a mapping entry whose value references `pigeon`.
...
`result` contains the same values as `birds`, but its keys have changed to `"derF"` and `"amliW"`.
...
----
<1> Amends the `default` value and sets property `age`.
<2> Defines a mapping entry with key `"Pigeon"` that implicitly amends the default value.
<3> Defines a new property called `diet`. Property `age` is inherited from the default value.
...
Note that Parrot kept its age because its prior self defined it explicitly, overriding any default.
...
For example, the inferred name for a module first loaded from `+https://apple.com/pkl/bird.pkl+` is `bird`.
...
A package is shareable archive of modules and resources that are published to the internet.

To import `package://example.com/mypackage@1.0.0#/my_module.pkl`, Pkl follows these steps:

1. Make HTTPS GET request to `https://example.com/mypackage@1.0.0` to retrieve the package's metadata.
2. From the package metadata, download the referenced zip archive, and validate checksum.
...
<5> `Parrot` defines a new property named `isExtinct`.
...
<4> `parrot` defines a new property named `isExtinct`.
...
A module import implicitly defines a new `local` property through which the imported module can be accessed.
...
[source,{pkl}]
----
import* "clusters/*.pkl" as allClusters // <1>
import* "users/*.pkl" <2>

clusters = import*("clusters/*.pkl") // <3>
----
...
clusters = new Mapping {
  ["clusters/us-west-1.pkl"] = import("clusters/us-west-1.pkl")
  ["clusters/us-west-2.pkl"] = import("clusters/us-west-2.pkl")
  ["clusters/us-west-3.pkl"] = import("clusters/us-west-3.pkl")
}
----
...
1. _Outside_ the language, by using the `--format` CLI option or `outputFormat` Gradle task property.
...
[source%parsed,{pkl}]
----
clusterFiles = read*("clusters/*.txt")
----

.index.pkl
[source%parsed,{pkl}]
----
clusterFiles = new Mapping {
  ["clusters/us-west-1.txt"] = read("clusters/us-west-1.txt")
  ["clusters/us-west-2.txt"] = read("clusters/us-west-2.txt")
  ["clusters/us-west-3.txt"] = read("clusters/us-west-3.txt")
}
----
...
[source%tested%error,{pkl}]
----
myValue = throw("I'm outta here!") // <1>
----
...
All of the following tests hold:
...
names = employees.toList().map((it) -> it.name) as List<String>
----
...
[source%tested,{pkl}]
----
matches = emailRegex.findMatchesIn("pigeon@example.com / barnowl@example.com / parrot@example.com")
...
<2> result: `List("pigeon@example.com", "barnowl@example.com", "parrot@example.com")`
<3> result: `List("pigeon, barn owl, parrot")`
...
map: StringMap<Int> = Map("Pigeon", 42, "Barn Owl", 21)
----
...
age = 5
friends {
  import("barnOwl.pkl") // <1>
}
----
<1> _barnOwl.pkl_ (not shown here) is guaranteed to amend _bird.pkl_
...
----
<1> shorthand for `age: unknown = 42`
...
project: Project = new {
  type = "open-source"
  contacts = Map("Dr. Pigeon", "drpigeon@example.com")
}
----
...
<2> Implicitly applies the amended `default` function and amends the returned object with property `age`.

The result is a mapping whose entry `"Pigeon"` has both `diet` and `age` set.
...
Each entry is an object with properties `name` and `age` keyed by name.

[source%tested,{pkl}]
----
namesAndAges = Map("Pigeon", 2, "Barn Owl", 12, "Parrot", 7)

birdsByName {
  for (_name, _age in namesAndAges) {
    [_name] {
      name = _name
      age = _age
    }
...
Note that `for` generators can generate elements and entries but not properties.
footnote:[More precisely, they cannot generate properties with a non-constant name.]
...
|`resources/{\*.yml,*.json}`
|Anything within the `resources` subdirectory that ends in `.yml` or `.json`. This pattern is equivalent to `resources/*.{yml,json}`.
...
[source,{pkl}]
----
/// A link is *not* generated for https://apple.com.
/// A link *is* generated for <https://apple.com>.
----
====
...
///
/// Has [name] and [age] properties and an [isOlderThan()] method.
class Bird {
  ...
  function isOlderThan(bird: Bird): Boolean = age > bird.age
}

/// An adult [Bird].
typealias Adult = Bird(age >= 18)
...
/// Creates a [Bird] with the given [_name] and age `0`.
function Infant(_name: String): Bird = new { name = _name; age = 0 }
----
...
1. The chain of objects amended to create `x`, ending in `x` itself, in reverse order.
   footnote:[All objects in this chain are instances of the same class,
...
To reduce the chance of naming collisions, `Any` does not define any property names.
footnote:[Method resolution searches the class inheritance rather than prototype chain.]
...
These dependencies can then imported by their simple name.
...
It can be imported using dependency notation, i.e. `import "@fruit/Apple.pkl"`.
At runtime, it will resolve to relative path `../fruit/Apple.pkl`.
