...
Object instantiation in Pkl always <<amending-objects, amends>> _some_ value.
The `new` keyword is a special case of amending where a contextual default value is amended. 
In Pkl, objects can be instantiated in three major ways:

* Amending some parent property or value directly.
* Invoking `new` with explicit type information, which always amends the default value of the specified type.
* Invoking `new` without type information, which amends a contextually inferred value.
...
gizmo: Gizmo // <1>
gizmoListing: Listing<Gizmo> // <2>
gizmoMapping: Mapping<String, Gizmo> // <3> 
----
<1> Without an explicit default value, this property has default value `new Gizmo { name = "doodad" }`
<2> With an explicit element type argument, this property's default value is equivalent to `new Listing<Gizmo> { default = new Gizmo { name = "doodad" } }`
<3> With an explicit value type argument, this property's default value is equivalent to `new Mapping<String, Gizmo> { default = (_) -> new Gizmo { name = "doodad" } }`

==== Explicit Parent

Explicit amending is covered in detail by <<amending-objects, Amending Objects>>.
Amending a property, entry, or value results in a new value with the same type as the parent.
When amending a property that has not been previously declared, an empty `Dynamic` object is used as the parent.
...
amendedProperty = (newProperty) { // <2> 
  hello = "there"
}
someMapping = new Mapping {
  ["greetings"] { // <3>
    hello = "everyone"
  }
}
----
<1> Amending a property.
<2> Amending a value.
<3> Amending a new `Mapping` entry.
...
Notably, creating a `Listing` element or assigning a `Mapping` value with an explicitly typed `new` ignores a collection's `default` value.

[source,{pkl}]
----
newProperty = new Dynamic { // <1>
  hello = "world"
}
someListing = new Listing {
  default {
    isDefault = true
  }
  new Dynamic { // <2>
...
The value will not have property `isDefault = true` as the `default` property of the `Listing` is not used.
<3> Assigning a `new` explicitly-typed value to a `Mapping` entry.
The value will not have property `isDefault = true` as the `default` property of the `Mapping` is not used.

==== Untyped `new`

When using the untyped `new` invocation, there is no explicit parent value to amend.
In these cases, Pkl infers the amend operation's parent value based on context.
In order of precedence:

1. When assigning to a declared property, the property's default value is amended (<<amend-null, including `null`>>).
2. Within a `Listing` or `Mapping`, the container's function-typed `default` property is applied to the corresponding index or key, respectively, to produce the value to be amended.
3. In module or object body context, when no type information is otherwise available, an empty `Dynamic` object is amended.
4. In other cases, type inference fails with the error message "Cannot tell which parent to amend".

Currently, the type information of a <<methods,method>> parameter is not used for type inference, which may lead to surprising type inference errors.
...
class Gizmo {
  ...
  function gizmofy(items: Listing<String>): Listing<String> = new {
    for (item in items) {
      "\(name):\(item)"
    }
  }
}
typedProperty: Gizmo = new { // <1>
  name = "doodad"
}
untypedProperty = new { // <2>
  hello = "world"
}
typedListing: Listing<Gizmo> = new {
  new { // <3>
    name = "thingy"
  }
}
...
typedMapping: Mapping<String, Gizmo> = new {
  default { entryKey ->
    name = entryKey
  }
  ["doohicky"] = new { // <5>
...
doodadGizmos = typedProperty.gizmofy(new { "widget"; "whatchamacallit" }) // <7>
----
<1> Assignment to a property with an explicitly declared type, amending `new Gizmo {}` (case 1 above).
<2> Assignment to an undeclared property in module context, amending `new Dynamic {}` (case 3 above).
<3> `Listing` element creation, amending implicit `default`, `new Gizmo {}` (case 2 above).
<4> `Listing` element creation, amending implicit `default`, `new Dynamic {}` (case 2 above).
<5> `Mapping` value assignment, amdending the result of applying `default` to `"doohicky"`, `new Gizmo { name = "doohicky" }` (case 2 above).
<6> `Mapping` value assignment _replacing_ the parent's entry, amending the result of applying `default` to `"doohicky"`, `new Gizmo { name = "doohicky" }` (case 2 above).
<7> Error: Cannot tell which parent to amend (case 4 above).
