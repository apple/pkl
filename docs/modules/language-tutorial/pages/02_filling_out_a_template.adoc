...
They only differ in their name, diet and taxonomy, so if you have already written out `bird`, you can say that `parrot` is just like `pigeon` except `name` is `"Parrot"`, `diet` is `"Berries"` the `taxonomy.order` is `"Psittaciformes"`.
