...
When external properties or environment variables need to vary per call, use the `evaluate`, `evaluateOutputValue`, or
`evaluateExpression` overloads that accept an `EvaluationContext`.
Create a context with `EvaluationContext.builder().addExternalProperty("name", "value").build()`,
and use `addEnvironmentVariable` to override environment variables.
The context overrides the evaluator's configured values for that evaluation only,
and the call does not reuse cached module or resource evaluation results that could contain stale
`read("prop:...")` or `read("env:...")` values.
An empty context also starts with fresh caches, and a context can be reused across calls.
