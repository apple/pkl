...
Like non-primitive values, types are encoded as MessagePack arrays, where the first slot designates the type of the type.
...
|`0x44`
|`<class>`
|<<class,Class>> of the module where the `module` type was used
|link:{uri-messagepack-bool}[bool]
...
|`0x45`
|`<class>`
|<<class,Class>> where the `this` type was used
|link:{uri-messagepack-bool}[bool]
|Whether the class is final or not
|
|
...
|Base type
|link:{uri-messagepack-array}[array]
|Array of link:{uri-messagepack-str}[str] value representations of the type's constraints
|
|
...
|`0x48`
|`<typealias>`
|<<typealias,Type Alias>> type declaration
|link:{uri-messagepack-array}[array]
|Array of type arguments
|`<type>`
