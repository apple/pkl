 ...
 *
 * <p>The key/value pairs are kept in an expanding flat object array with keys at even indices and
 ...
 *
 * <p>When the hash table needs to be constructed, the field {@link #hashArray} becomes a new hash
 * array where an entry of 0 means no hit and otherwise denotes the entry number in the {@link
 * #entries} array. The hash array is interpreted as an actual byte array if the indices fit within
 * 8 bit, or as an array of short values if the indices fit within 16 bit, or as an array of integer
 * values in other cases.
 *
 * <p>Hash collisions are handled by chaining a linked list of {@link CollisionLink} objects that
 * take the place of the values in the {@link #entries} array.
 *
 * <p>Removing entries will put {@code null} into the {@link #entries} array. If the occupation of
 * the map falls below a specific threshold, the map will be compressed via the {@link
 * #maybeCompress(int)} method.
 */
final class EconomicMapImpl<K, V> implements EconomicMap<K, V>, EconomicSet<K> {
  ...
  /**
   * The strategy used for comparing keys or {@code null} for denoting special strategy {@link
  ...
  @TruffleBoundary
  public static <K, V> EconomicMapImpl<K, V> create(
      Equivalence strategy, int initialCapacity, boolean isSet) {
    return intercept(new EconomicMapImpl<>(strategy, initialCapacity, isSet));
  }

  @TruffleBoundary
  public static <K, V> EconomicMapImpl<K, V> create(
      Equivalence strategy, UnmodifiableEconomicMap<K, V> other, boolean isSet) {
    return intercept(new EconomicMapImpl<>(strategy, other, isSet));
  }

  @TruffleBoundary
  public static <K, V> EconomicMapImpl<K, V> create(
      Equivalence strategy, UnmodifiableEconomicSet<K> other, boolean isSet) {
    return intercept(new EconomicMapImpl<>(strategy, other, isSet));
  }
  ...
  private boolean compareKeys(Object key, @Nullable Object entryKey) {
    if (key == entryKey) {
      return true;
    }
  ...
  private int getHashArray(int index) {
    if (entries.length < LARGE_HASH_THRESHOLD) {
      ...
      return (hashArray[adjustedIndex] & 0xFF)
          | ((hashArray[adjustedIndex + 1] & 0xFF) << 8)
          | ((hashArray[adjustedIndex + 2] & 0xFF) << 16)
          | ((hashArray[adjustedIndex + 3] & 0xFF) << 24);
    }
  }
  ...
  private int findAndRemoveWithCollision(
      Object key, CollisionLink initialEntryValue, int initialIndexValue) {
    int index;
    Object entryKey;
    CollisionLink entryValue = initialEntryValue;
  ...
  private void grow() {
    ...
    if ((entriesLength < LARGE_HASH_THRESHOLD && newEntries.length >= LARGE_HASH_THRESHOLD)
        || (entriesLength < VERY_LARGE_HASH_THRESHOLD
  ...
  private int maybeCompress(int nextIndex) {
    if (entries.length != INITIAL_CAPACITY << 1
        && deletedEntries >= (totalEntries >> 1) + (totalEntries >> 2)) {
  ...
  private void putHashEntry(Object key, int entryIndex, boolean rehashOnCollision) {
    ...
    if (oldIndex != -1) {
      assert entryIndex != oldIndex
          : "this cannot happen and would create an endless collision link cycle";
      if (value instanceof CollisionLink) {
        CollisionLink collisionLink = (CollisionLink) value;
  ...
  private @Nullable Object getKey(int index) {
    return entries[index << 1];
  }

  private void setKey(int index, @Nullable Object newValue) {
  ...
  private void setRawValue(int index, @Nullable Object newValue) {
    entries[(index << 1) + 1] = newValue;
  }

  private @Nullable Object getRawValue(int index) {
    return entries[(index << 1) + 1];
  }

  private @Nullable Object getValue(int index) {
