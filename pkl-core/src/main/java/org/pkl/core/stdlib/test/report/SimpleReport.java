...
public final class SimpleReport implements TestReport {
  ...
  public void report(TestResults results, Writer writer) throws IOException {
    ...
    builder.append("module ").append(results.moduleName);

    builder.append(results.failed() ? " ❌" : " ✅");

    var statsLine = makeStatsLine(results.totalTests(), results.totalFailures(), results.failed());
    builder.append(statsLine);

    builder.append(" (").append(results.displayUri).append(")");
    builder.append("\n");
  ...
  private void reportResults(TestSectionResults section, StringBuilder builder) {
    if (!section.getResults().isEmpty()) {
      ...
      builder.append(section.failed() ? " ❌" : " ✅");

      var statsLine =
          makeStatsLine(section.totalTests(), section.totalFailures(), section.failed());
      builder.append(statsLine);
  ...
  private void reportResult(TestResult result, StringBuilder builder) {
    ...
    if (result.isExampleWritten()) {
      ...
      builder.append(result.isFailure() ? " ❌" : " ✅");

      if (!result.isExample) {
        var statsLine =
            makeStatsLine(result.totalAsserts(), result.getFailures().size(), result.isFailure());
        builder.append(statsLine);
      }
  ...
  private String makeStatsLine(int total, int failed, boolean isFailed) {
    ...
    if (isFailed) {
      line = String.format(" %.1f%% pass [%d passed, %d failed]", pct_passed, passed, failed);
    } else {
      line = String.format(" %.1f%% pass [%d passed]", pct_passed, passed);
    }
