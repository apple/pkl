...
public final class ParserNodes {
  ...
  /**
   * Wrap a generic node into its typed {@code Node} counterpart, or return {@code null} if its type
   * has no typed counterpart. Backs {@code pkl.syntax#GenericNode.toNode}.
   */
  static @Nullable VmTyped toTypedNodeOrNull(VmTyped nodeVm) {
    var genericVm =
        isFullyBacked(nodeVm)
            ? nodeVm
            : convertNode(
                SyntaxNodes.convertVmToNode(nodeVm, SyntaxNodes.ZERO_SPAN),
                SyntaxNodes.EMPTY_SOURCE,
                null);
    var nodeType = ((GenericNodeData) genericVm.getExtraStorage()).node.type;
    if (nodeType.isExpression()) {
      return wrapExpr(genericVm);
    }
    if (nodeType.isType()) {
      return wrapType(genericVm);
    }
    return switch (nodeType) {
      case MODULE -> moduleNodeFactory.create(genericVm);
      case MODULE_DECLARATION -> moduleDeclarationNodeFactory.create(genericVm);
      case AMENDS_CLAUSE, EXTENDS_CLAUSE -> extendsOrAmendsClauseNodeFactory.create(genericVm);
      case IMPORT -> importNodeFactory.create(genericVm);
      case CLASS -> classNodeFactory.create(genericVm);
      case TYPEALIAS -> typeAliasNodeFactory.create(genericVm);
      case CLASS_BODY -> classBodyNodeFactory.create(genericVm);
      case CLASS_PROPERTY -> classPropertyNodeFactory.create(genericVm);
      case CLASS_METHOD -> classMethodNodeFactory.create(genericVm);
      case OBJECT_BODY -> objectBodyNodeFactory.create(genericVm);
      case OBJECT_PROPERTY -> objectPropertyNodeFactory.create(genericVm);
      case OBJECT_METHOD -> objectMethodNodeFactory.create(genericVm);
      case OBJECT_ENTRY -> objectEntryNodeFactory.create(genericVm);
      case OBJECT_SPREAD -> objectSpreadNodeFactory.create(genericVm);
      case MEMBER_PREDICATE -> memberPredicateNodeFactory.create(genericVm);
      case FOR_GENERATOR -> forGeneratorNodeFactory.create(genericVm);
      case WHEN_GENERATOR -> whenGeneratorNodeFactory.create(genericVm);
      case ANNOTATION -> annotationNodeFactory.create(genericVm);
      case PARAMETER -> parameterNodeFactory.create(genericVm);
      case TYPE_PARAMETER -> typeParameterNodeFactory.create(genericVm);
      case IDENTIFIER -> identifierNodeFactory.create(genericVm);
      case QUALIFIED_IDENTIFIER -> qualifiedIdentifierNodeFactory.create(genericVm);
      case DOC_COMMENT -> docCommentNodeFactory.create(genericVm);
      default -> null;
    };
  }

  /**
   * Tells if {@code nodeVm} and all its descendants carry the parse-time state that the typed
   * accessors read.
   *
   * <p>A tree can be backed only in part: {@code transform} rebuilds the ancestors of a replaced
   * node, but keeps the replacement itself as its child, and a replacement constructed or amended
   * in Pkl brings no state of its own.
   */
  private static boolean isFullyBacked(VmTyped nodeVm) {
    if (!nodeVm.hasExtraStorage()) {
      return false;
    }
    var childrenVm = ((GenericNodeData) nodeVm.getExtraStorage()).childrenVm;
    for (var i = 0; i < childrenVm.getLength(); i++) {
      if (!isFullyBacked((VmTyped) childrenVm.get(i))) {
        return false;
      }
    }
    return true;
