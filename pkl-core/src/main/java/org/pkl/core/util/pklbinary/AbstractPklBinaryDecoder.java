...
public abstract class AbstractPklBinaryDecoder {
  ...
  private void assertLength(PklBinaryTypes type, int len, int expected) {
    if (len < expected) {
  ...
  protected record DecodedObjectMember(PklBinaryTypes type, Object key, Object value) {}
  ...
  private Object decodeNonPrimitive() throws IOException {
    ...
    var code = PklBinaryTypes.fromInt(codeInt);
    if (code == null) {
      throw new DecodeException("Unrecognized code 0x%x", (byte) codeInt);
    }

    return switch (code) {
      case CODE_OBJECT -> decodeObject(len);
      case CODE_MAP -> decodeMap(len);
      case CODE_MAPPING -> decodeMapping(len);
      case CODE_LIST -> decodeList(len);
      case CODE_LISTING -> decodeListing(len);
      case CODE_SET -> decodeSet(len);
      case CODE_DURATION -> decodeDuration(len);
      case CODE_DATASIZE -> decodeDataSize(len);
      case CODE_PAIR -> decodePair(len);
      case CODE_INTSEQ -> decodeIntSeq(len);
      case CODE_REGEX -> decodeRegex(len);
      case CODE_CLASS -> decodeClass(len);
      case CODE_TYPEALIAS -> decodeTypeAlias(len);
      case CODE_FUNCTION -> decodeFunction(len);
      case CODE_BYTES -> decodeBytes(len);
      default -> throw new DecodeException("Unrecognized object code %s", code);
    };
  }

  private Object decodeObject(int len) throws IOException {
    assertLength(PklBinaryTypes.CODE_OBJECT, len, 3);
  ...
  private Object decodeMap(int len) throws IOException {
    assertLength(PklBinaryTypes.CODE_MAP, len, 1);
    currPath.push("'map");
  ...
  private Object decodeMapping(int len) throws IOException {
    assertLength(PklBinaryTypes.CODE_MAPPING, len, 1);
  ...
  private Object decodeList(int len) throws IOException {
    assertLength(PklBinaryTypes.CODE_LIST, len, 1);
    currPath.push("'list");
  ...
  private Object decodeListing(int len) throws IOException {
    assertLength(PklBinaryTypes.CODE_LISTING, len, 1);
  ...
  private Object decodeSet(int len) throws IOException {
    assertLength(PklBinaryTypes.CODE_SET, len, 1);
    currPath.push("'set");
  ...
  private Object decodeDuration(int len) throws IOException {
    assertLength(PklBinaryTypes.CODE_DURATION, len, 2);
  ...
  private Object decodeDataSize(int len) throws IOException {
    assertLength(PklBinaryTypes.CODE_DATASIZE, len, 2);
  ...
  private Object decodePair(int len) throws IOException {
    assertLength(PklBinaryTypes.CODE_PAIR, len, 2);
    currPath.push("'pair");
    currPath.push("'first");
    var first = doDecode();
    currPath.pop();
  ...
  private Object decodeIntSeq(int len) throws IOException {
    assertLength(PklBinaryTypes.CODE_INTSEQ, len, 3);
  ...
  private Object decodeRegex(int len) throws IOException {
    assertLength(PklBinaryTypes.CODE_REGEX, len, 1);
    currPath.push("'regex");
  ...
  private Object decodeClass(int len) throws IOException {
    assertLength(PklBinaryTypes.CODE_CLASS, len, 2);
    currPath.push("'class");
  ...
  private Object decodeTypeAlias(int len) throws IOException {
    assertLength(PklBinaryTypes.CODE_TYPEALIAS, len, 2);
  ...
  private Object decodeFunction(int len) throws IOException {
    assertLength(PklBinaryTypes.CODE_FUNCTION, len, 0);
  ...
  private Object decodeBytes(int len) throws IOException {
    assertLength(PklBinaryTypes.CODE_BYTES, len, 1);
    currPath.push("'bytes");
  ...
  protected class ObjectDecodeIterator extends DecodeIterator<DecodedObjectMember> {
    ...
    DecodedObjectMember getNext() throws IOException {
      ...
      var memberCode = PklBinaryTypes.fromInt(memberCodeInt);
      if (memberCode == null) {
