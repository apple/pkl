...
class KotlinCodeGenerator(
    ...
    private val propertiesFile: String
        get() {
            ...
            return StringWriter()
                .apply {
                    props.store(this, "Kotlin mappings for Pkl module `${moduleSchema.moduleName}`")
                }
                .toString()
        }
    ...
    val kotlinFile: String
        get() {
            ...
            for (pClass in moduleSchema.classes.values) {
                moduleType.addType(
                    generateTypeSpec(pClass, moduleSchema).ensureSerializable().build()
                )
            }

            // generate append method for module classes w/o parent class; reuse in subclasses and
            // nested
            // classes
                ...
                if (isGenerateClass) {
                    moduleType.addType(
                        TypeSpec.companionObjectBuilder()
                            .addFunction(
                                appendPropertyMethod()
                                    .addModifiers(appendPropertyMethodModifier)
                                    .build()
                            )
                            .build()
                    )
            ...
            for (typeAlias in moduleSchema.typeAliases.values) {
                if (typeAlias.aliasedType is PType.Alias) {
                    ...
                    if (
                        CodeGeneratorUtils.isRepresentableAsEnum(
                        ...
                        // generate top-level type alias (Kotlin doesn't support nested type
                        // aliases)
                        fileSpec.addTypeAlias(generateTypeAliasSpec(typeAlias).build())
                    }
                }
    ...
    private fun generateTypeSpec(pClass: PClass, schema: ModuleSchema): TypeSpec.Builder {
        ...
        fun generateCopyMethod(
            parameters: Map<String, PClass.Property>,
            isOverride: Boolean
        ): FunSpec {
        ...
        fun generateCopyMethods(typeBuilder: TypeSpec.Builder) {
            ...
            for (currClass in generateSequence(pClass) { it.superclass }) {
                ...
                if (currParameters.size < prevParameterCount) {
                    val isOverride =
                        currClass !== pClass || superclass != null && properties.isEmpty()
            ...
            annotations
                ...
                ?.let { deprecation ->
                    ...
                    (deprecation["message"] as String?)?.let {
                        builder.addMember("message = %S", it)
                    }
        ...
        fun generateProperty(propertyName: String, property: PClass.Property): PropertySpec {
            ...
            val builder =
                PropertySpec.builder(propertyName, typeName).initializer("%L", propertyName)
        ...
        for ((enumConstantName, pklName) in enumConstantToPklNames) {
            builder.addEnumConstant(
                ...
                TypeSpec.anonymousClassBuilder()
                    .addSuperclassConstructorParameter("%S", pklName)
                    .build()
            )
        }
                ...
                when (val classInfo = pClass.info) {
                    ...
                    PClassInfo.Pair ->
                        KOTLIN_PAIR.parameterizedBy(
                            if (typeArguments.isEmpty()) ANY_NULL
                            else typeArguments[0].toKotlinPoetName(),
                            if (typeArguments.isEmpty()) ANY_NULL
                            else typeArguments[1].toKotlinPoetName()
                        )
                    ...
                    "pkl.base#Uri" -> URI
                        ...
                        when {
                            ...
                            typeArguments.isEmpty() -> {
                                // no type arguments provided for a type alias with type parameters
                                // -> fill in
