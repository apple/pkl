...
constructor(
    ...
    /**
     * Output files for the modules to be evaluated. Returns `null` if `options.outputPath` is
     * `null`. Multiple modules may be mapped to the same output file, in which case their outputs
     * are concatenated with [CliEvaluatorOptions.moduleOutputSeparator].
     */
     ...
     * each module's `output.files` are written to the module's
     * [output directory][outputDirectories]. Otherwise, each module's `output.text` is written to
     * [consoleWriter] (which defaults to standard out).
     *
        ...
        return moduleUris.associateWith { uri ->
            val moduleDir: String? =
                IoUtils.toPath(uri)?.let {
                    workingDir.relativize(it.parent).toString().ifEmpty { "." }
                }
            val moduleKey =
                try {
    ...
    private fun writeOutput(builder: EvaluatorBuilder) {
        ...
        evaluator.use {
            ...
            if (outputFiles != null) {
                ...
                // YamlRenderer produces empty output if `isStream` is true and `output.value` is
                // empty
                // collection
                ...
                for ((moduleUri, outputFile) in outputFiles) {
                    ...
                    val output =
                        evaluator.evaluateExpressionString(moduleSource, options.expression)
    ...
    private fun writeMultipleFileOutput(builder: EvaluatorBuilder) {
        ...
        for ((moduleUri, outputDir) in outputDirs) {
            ...
            for ((pathSpec, fileOutput) in output) {
                ...
                val realPath =
                    if (resolvedPath.exists()) resolvedPath.toRealPath() else resolvedPath
