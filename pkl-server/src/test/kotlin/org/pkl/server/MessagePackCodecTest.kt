...
class MessagePackCodecTest {
                    ...
                    Project(
                        ...
                        dependencies =
                            mapOf(
                                            ...
                                            mapOf(
                                                "bar" to
                                                    Project(
                                                        ...
                                                        packageUri =
                                                            URI(
                                                                "package://localhost:12110/bar@1.1.0"
                                                            ),
                                ...
                                "baz" to
                                    RemoteDependency(
                                        URI("package://localhost:12110/baz@1.1.0"),
                                        Checksums("abc123")
                                    )
                            )
                    )
    ...
    fun `round-trip ReadModuleRequest`() {
        roundtrip(
            ReadModuleRequest(requestId = 123, evaluatorId = 456, uri = URI("some/module.pkl"))
        )
    }
    ...
    fun `round-trip ListModulesRequest`() {
        roundtrip(
            ListModulesRequest(requestId = 135, evaluatorId = 246, uri = URI("foo:/bar/baz/biz"))
        )
    }
    ...
    fun `round-trip ListResourcesRequest`() {
        roundtrip(
            ListResourcesRequest(requestId = 987, evaluatorId = 1359, uri = URI("bar:/bazzy"))
        )
    }
