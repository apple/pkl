...
internal class MessagePackDecoder(private val unpacker: MessageUnpacker) : MessageDecoder {
            ...
            try {
                ...
                if (arraySize != 2) {
                    throw DecodeException(
                        "Malformed message header (expected size 2, but got $arraySize)."
                    )
                }
                unpacker.unpackInt()
            ...
            when (code) {
                MessageType.CREATE_EVALUATOR_REQUEST.code -> {
                    CreateEvaluatorRequest(
                        ...
                        allowedModules =
                            map.unpackStringListOrNull("allowedModules")?.map(Pattern::compile),
                        allowedResources =
                        ...
                        clientResourceReaders =
                            map.unpackResourceReaderSpec("clientResourceReaders"),
    ...
    private fun Map<Value, Value>.unpackIntValue(key: String): Int =
        get(key).asIntegerValue().asInt()
