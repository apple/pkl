...
internal class ClientModuleKeyFactory(
  ...
  private val resolver: ModuleKeys.External.Resolver =
    ModuleKeys.External.Resolver(transport, evaluatorId)

  override fun create(uri: URI): Optional<ModuleKey> =
    when (uri.scheme) {
      in schemes -> {
        val readerSpec = readerSpecs.find { it.scheme == uri.scheme }!!
        val moduleKey = ModuleKeys.external(uri, readerSpec, resolver)
