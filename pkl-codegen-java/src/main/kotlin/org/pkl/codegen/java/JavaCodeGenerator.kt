...
class JavaCodeGenerator(
    ...
    private val propertyFileName: String
        get() =
            "resources/META-INF/org/pkl/config/java/mapper/classes/${schema.moduleName}.properties"
        ...
        get() {
            ...
            for (pClass in schema.classes.values) {
                props["$PROPERTY_PREFIX${pClass.qualifiedName}"] =
                    pClass.toJavaPoetName().reflectionName()
            }
            return StringWriter()
    ...
    val javaFile: String
        get() {
            ...
            for (typeAlias in schema.typeAliases.values) {
                ...
                if (
                    CodeGeneratorUtils.isRepresentableAsEnum(typeAlias.aliasedType, stringLiterals)
                ) {
                    moduleClass.addType(generateEnumTypeSpec(typeAlias, stringLiterals).build())
                }
            }
            // generate static append method for module classes w/o parent class; reuse in
            // subclasses and
            // nested classes
    ...
    private fun generateTypeSpec(pClass: PClass, schema: ModuleSchema): TypeSpec.Builder {
        ...
        val superclass =
            pClass.superclass?.takeIf {
                it.info != PClassInfo.Typed && it.info != PClassInfo.Module
            }
        val superProperties =
            superclass
                ?.let { renameIfReservedWord(it.allProperties) }
                    ...
                    .addModifiers(
                        when {
                            ...
                            allProperties.isNotEmpty() ->
                                Modifier.PUBLIC // if `false`, has no state
        ...
        fun generateEqualsMethod(): MethodSpec {
            ...
            for ((propertyName, property) in allProperties) {
                val accessor =
                    if ((property.type as? PType.Class)?.pClass?.info == PClassInfo.Regex)
                        "\$N.pattern()"
                    else "\$N"
        ...
        fun generateToStringMethod(): MethodSpec {
            ...
            builder
                ...
                .addStatement(
                    "builder.append(\$T.class.getSimpleName()).append(\" {\")",
                    javaPoetClassName
                )
        ...
        fun generateField(propertyName: String, property: PClass.Property): FieldSpec {
            ...
            val hasJavadoc =
                docComment != null &&
                    codegenOptions.generateJavadoc &&
                    !codegenOptions.generateGetters
            if (hasJavadoc) {
            ...
            if (isModuleClass) {
                ...
                val modulePropertiesWithMatchingType =
                    schema.moduleClass.allProperties.values.filter { property ->
                        ...
                        while (
                            propertyType is PType.Constrained || propertyType is PType.Nullable
                        ) {
                ...
                if (modulePropertiesWithMatchingType.size == 1) {
                    // exactly one module property has this type -> make it available for direct
                    // injection
                    // (potential improvement: make type available for direct injection if it occurs
                    // exactly
                    ...
                    builder.addAnnotation(
                        AnnotationSpec.builder(
                            ...
                            // use "value" instead of "prefix" to entice JavaPoet to generate a
                            // single-line
                            // annotation
                            // that can easily be filtered out by JavaCodeGeneratorTest.`spring boot
                            // config`
                            .addMember(
                                "value",
                                "\$S",
                                modulePropertiesWithMatchingType.first().simpleName
                            )
                            .build()
                    )
                }
        ...
        fun generateClass(): TypeSpec.Builder {
            ...
            // generate fields, plus getter methods and either setters or `with` methods in
            // alternating
            // order
            // `with` methods also need to be generated for superclass properties so that return
            // type is
            // self type
    ...
    private fun PClass.toJavaPoetName(): ClassName {
        ...
        val packageName = if (index == -1) "" else moduleName.substring(0, index)
        val moduleClassName =
            moduleName.substring(index + 1).replaceFirstChar { it.titlecaseChar() }
    ...
    private fun PType.toJavaPoetName(nullable: Boolean = false, boxed: Boolean = false): TypeName =
        when (this) {
                    ...
                    PClassInfo.Version -> VERSION.nullableIf(nullable)
                        ...
                        when {
                            !classInfo.isStandardLibraryClass ->
                                pClass.toJavaPoetName().nullableIf(nullable)
                            else ->
                                throw JavaCodeGeneratorException(
            ...
            is PType.Union ->
                ...
                // should never encounter PType.TypeVariableNode because it can only occur in stdlib
                // classes
