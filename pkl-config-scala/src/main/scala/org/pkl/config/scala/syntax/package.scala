...
/**
 * Entry point for Scala-specific extensions to PKL configuration, enabling type conversions and
 * syntax improvements that align PKL's configuration model with Scala types and structures.
 *
 * The `syntax` package object introduces two main extensions:
 *
 *   1. `forScala`: Enhances the PKL evaluation stack by adding Scala-specific type conversions and
 *      converter factories, making it possible to work seamlessly with Scala types.
 *   2. `Config.to`: Provides a type-safe `Config` conversion method.
 */
package object syntax {

  /**
   * Extension for `ValueMapperBuilder`, enabling Scala-specific type conversions and factories.
   *
   * Adds conversions from `ScalaConversions` and converter factories from `ScalaConverterFactories`
   * to the evaluation stack, allowing PKL to handle Scala-native types effectively.
   *
   * @example
  ...
  /**
   * Extension for `ConfigEvaluatorBuilder`, enabling Scala-specific type handling in the evaluator.
   *
   * This method sets up a `ConfigEvaluatorBuilder` with a `ValueMapperBuilder` that has been
   * extended with Scala conversions, enabling the evaluator to process Scala-specific types in PKL
   * configurations.
   *
   * @example
  ...
  /**
   * Extension for `ConfigEvaluator`, applying Scala-specific type conversions to the evaluator.
   *
   * Builds a `ConfigEvaluator` with a Scala-aware `ValueMapper`, allowing for seamless conversion
   * of configuration values to Scala types.
   *
   * @example
  ...
  /**
   * Extension for `Config`, adding a type-safe `to` method to retrieve values as Scala types.
   *
   * The `to[T]` method provides an intuitive way to retrieve values from a PKL `Config` as specific
   * Scala types. For nullable PKL values, declare the target as `Option[T]`; otherwise a
   * `ConversionException` propagates from the underlying `ValueMapper`.
   *
   * @param ct
   *   Implicit `ClassTag` of the target type `T`.
   *
   * @throws ConversionException
   *   if the value cannot be converted to the specified type `T`.
   *
   * @example
   *   Retrieving a value:
   *   {{{
   * val myPklConfig: Config = // load or build a PKL config
   * val config: MyCaseClass = myPklConfig.to[MyCaseClass]
   *   }}}
   */
