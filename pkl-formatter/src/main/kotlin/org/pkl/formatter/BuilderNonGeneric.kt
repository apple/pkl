...
class BuilderNonGeneric(sourceText: String) : ParserVisitor<FormatNode> {
  ...
  override fun visitMultiLineStringLiteralExpr(expr: Expr.MultiLineStringLiteralExpr): FormatNode {
    return withPrefixesAndSuffixes(expr) {
      MultilineStringGroup(expr.endDelimiterSpan.colBegin(), expr.parts.map(::visitStringPart))
    }
  }
  ...
  override fun visitObjectBody(body: ObjectBody): FormatNode {
    return withPrefixesAndSuffixes(body) {
      ...
      if (paramNodes.isEmpty() && members.isEmpty()) {
        ...
        val separator =
          if (body.span().lineEnd() > body.span().lineBegin()) ForceLine else SpaceOrLine
  ...
  private fun Node.visit(): FormatNode = accept(this@BuilderNonGeneric)
