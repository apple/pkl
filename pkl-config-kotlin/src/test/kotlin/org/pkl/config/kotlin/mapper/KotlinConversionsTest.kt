...
class KotlinConversionsTest {
    ...
    fun pRegexToKotlinRegex() {
        val result =
            KotlinConversions.pRegexToKotlinRegex.converter.convert(
                Pattern.compile("(?i)\\w*"),
                mapper
            )
        assertThat(result.pattern).isEqualTo("(?i)\\w*")
    ...
    fun pIntToUInt() {
        ...
        assertThrows<ConversionException> {
            KotlinConversions.pIntToUInt.converter.convert(-1, mapper)
        }
    }

    @Test
    fun pIntToUShort() {
        assertThat(KotlinConversions.pIntToUShort.converter.convert(0, mapper))
            .isEqualTo(0.toUShort())

        assertThat(
                KotlinConversions.pIntToUShort.converter.convert(UShort.MAX_VALUE.toLong(), mapper)
            )
