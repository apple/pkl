...
object PackageServer {
    ...
    fun ensureStarted() =
        synchronized(this) {
            if (!started) {
                // Crude hack to make sure that parrallel tests don't try and use each others mock
                // server
                // otherwise you get flaky tests when a server instance is shutdown by one set of
                // tests
                // while another set of tests is still relying on it.
                // Side effect is that tests that spin up a mock package server are now serialised,
                // rather
                // than running in parrallel. But that seems like a reasonable tradeoff to avoid
                // flaky
                // tests.
                for (i in 1..20) {
                    try {
                ...
                println(
                    "Unable to start package server! This will probably result in a test failures"
                )
            }
        }
}
