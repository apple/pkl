...
class JavaCodeGeneratorsTest : AbstractTest() {
  ...
  fun `generate code`() {
    writeBuildFile()
    writePklFile()
    // lay down the gradle.properties file with proxy settings
    // for builds to be able to download dependencies from the internet
    // when they are running using MOES
    val rioExecutorEnvVar = System.getenv("RIO_EXECUTOR")
    if (rioExecutorEnvVar != null && rioExecutorEnvVar.equals("moes")){
